...
cv::Mat Viewer::PlotFrameImage() {
    ...
    for (size_t i = 0; i < current_frame_->left_features_.size(); i++) {
        if (current_frame_->left_features_[i]->mp_.lock()) {
            auto feat = current_frame_->left_features_[i];
            cv::circle(img_out, feat->kp_pos_.pt, 2, cv::Scalar(0, 250, 0),
                        2);
        }
    }
    return img_out;
...
cv::Mat Viewer::ShowImgKpForDebug(Frame::Ptr frame) {
    cv::Mat img_out;
    cv::cvtColor(current_frame_->left_img_, img_out, CV_GRAY2BGR);
    for (size_t i = 0; i < current_frame_->left_features_.size(); i++) {
        if (current_frame_->left_features_[i]->mp_.lock()) {
...
void Viewer::DrawFrame(Frame::Ptr frame, const float* color) {
    Sophus::SE3d Twc = frame->pose_.inverse();
    const float sz = 1.0;
